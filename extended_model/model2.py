...
# gaussian distribution (to find point inside the lobule)
def sample_gaussian_in_polygon(polygon, center, sigma):
    minx, miny, maxx, maxy = polygon.bounds
    prob_point = np.random.uniform(0,1)
    while True:
...
# -------------- evolution loop --------------------
    for i in range(iter):
        ...
        # let's check for lesion reproduction
        born_seed = False # no new lesions have seeded (yet)
        for e in range(1, count+1):
            if ages[f'age_{e}'] == 14.0:
                c_l_min = state[f'c_l_{e}']
                min_state[f'c_l_{e}_min'] = c_l_min
            ...
            if ages[f'age_{e}'] >= 14.0:
                ...
                if np.random.uniform(0,1) < P_rep: # new lesion seeded
                        born_seed = True
                        count += 1
                        # let's randomly select a new origin
                        point_born = sample_gaussian_in_polygon(central_region, origins[f'origin_{e}'], 100)
                        # let's initialise the new lesion and its variables
                        ...
                        i_encaps[f'encaps_{count}'] = 0
                
                    
        # let's check for coalescence
        ...
        # if there has been any mass change we must update g_global
        if born_seed or coalesced_seed: 
