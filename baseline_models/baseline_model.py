...
# calcification field initialisation
def create_calcium(region_polygon, x, y, origin):
    c_cal = np.zeros((x,y))
    (origin_x, origin_y) = (origin)
    origin_x = round(origin_x)
    origin_y = round(origin_y)
    c_cal[origin_x, origin_y] = 0.5 # we plant the calcium seed at the origin
    return c_cal
...
def calcium_evolution(x, y, dt, dx, c_cal, c_l, k_cal, D_cal):
    calcium_rate = k_cal * c_l * c_cal  # we want the calcification term to start from the middle of the lesion and grow. 

    # diffusion term
    # for the calcification to grow from the centre of the lesion to the boundaries, it must have a small diffusion
    cal_dif = compute_diffusion_flux(x, y, D_cal, c_cal, np.ones((x,y)), dx)
    
    # time integration
    c_cal_new = c_cal + (dt * (calcium_rate + cal_dif))
    c_cal_new[c_cal_new < 1e-4] = 0.0
    
    return c_cal_new, calcium_rate, cal_dif
...
def calcul_radius(m, dx):  # sum of tissue concentration
...
def model_evolution_loop(iter, params):
    ...
    k_cal = params["k_cal"]; D_cal = params["D_cal"]
   ...
   # -------------------- General --------------------
    ...
    r_cc_1 = np.zeros(iter) # radius of lesion + calcification
    ...
    cfl_vec = np.zeros(iter) # CFL coefficient at each iteration
    overshoot_vec = np.zeros(iter) # correction vector at each iteration
    n_cells_corrected_vec = np.zeros(iter) # number of cells corrected at each iteration
    ...
    radius = {'r_1' : r_1}
        ...
        # radius of lesion + calcification
    radius_c = {'r_c_1' : r_cc_1}
    ...
    phases = {'phase_1': np.zeros(iter, dtype=int)}
        ...
        # to save the variables at timepoints of interest
    c_l_time = {}
    c_cal_time = {}
    c_f_time = {}
...
# -------------- evolution loop --------------------
    for i in range(iter):
        ...
        # 2) Let's check for a change in "state" -> if encapsulation is completed
        window_size = 20000
        ...
        for e in range(1, count + 1):
            ...
            if encap_counter[f'encap_counter_{e}'] >= 10: # encapsulation confirmed
                ...
                origin_e = origins[f'origin_{e}'] 
                # calcification initialisation
                c_cal_seed = create_calcium(central_region, lx, ly, origin_e)
                calcification[f'c_cal_{e}'] = c_cal_seed
                new_les = np.maximum(state[f'c_l_{e}'] - c_cal_seed, 0.0)
                state[f'c_l_{e}'] = new_les
                seed_necrosis = True
           
        # if there has been any mass change (calcification seed planted) we must update g_global
        if seed_necrosis: 
            ...
            if state[key].any():                     
                ...
                if (age_e_curr >= 14.0): # we only integrate if the lesion is over 14 days of age                            
                    ...
                    if encapsulation[f'encap_{e}'] == True:
                        c_cal_new, cal_rate, cal_dif = calcium_evolution(lx, ly, dt, dx, c_cal_curr, c_l_e_curr, k_cal, D_cal)
                        cal_dif_vec[i] = np.max(np.abs(cal_dif))
                    else: 
        ...
        overshoot_vec[i] = np.max(total_mass)
        n_cells_corrected_vec[i] = np.sum(overshoot_mask) 
        ...
        for e in range(1, count + 1):
            ...
            radius_c_e = radius_c[f'r_c_{e}']
            radius_c_e[i] = calcul_radius(calcification[f'c_cal_{e}'], dx)
            radius_c[f'r_c_{e}'] = radius_c_e
        ...
        # 8) let's store the stability conditions (CFL)
        gx, gy = np.gradient(state['c_l_1'], dx)
        grad_l = np.sqrt(gx**2 + gy**2)
        
        # coeficients 
        D_kin = xi * grad_l* g_global + 1e-12
        D_tax = xi_tax * g_global * c_f1
        Dmax = max(D_l, np.max(D_kin), D_cal, np.max(D_tax))
        cfl = 4*dt * Dmax / (dx**2)
        cfl_vec[i] = cfl
    ...
    return state, ages, origins, radius, radius_c, full_radius, calcification, c_f1, max_grad, DIF_kin_vector, DIF_tax_vector, prol_vector, cal_dif_vec, phases, cfl_vec, overshoot_vec, n_cells_corrected_vec, c_l_time, c_cal_time, c_f_time
